...
const AgencyPage = () => {
  ...
  useEffect(() => {
    const fetchAgencyData = async () => {
      ...
      try {
        const response = await fetch(`https://cet-titz.onrender.com/api/agencies/${id}`);
        if (response.ok) {
    ...
    const fetchFeaturedAgencies = async () => {
      try {
        const response = await fetch(`https://cet-titz.onrender.com/api/agencies/featured`);
        if (response.ok) {
          const data = await response.json();
  ...
  return (
    <div className="container mx-auto py-16">
      ...
      <img
        src={`https://cet-titz.onrender.com/${agency.image}`}
        alt={agency.name}
        className="mb-4"
      />
